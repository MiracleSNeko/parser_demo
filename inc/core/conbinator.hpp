...
constexpr auto operator<<(Parser1&& parser1, Parser2&& parser2)
{
    return Combine(std::forward(parser1), std::forward(parser2), [](const auto& lhs, const auto& rhs) { return lhs; });
}
...
constexpr auto operator>>(Parser1&& parser1, Parser2&& parser2)
{
    return Combine(std::forward(parser1), std::forward(parser2), [](const auto& lhs, const auto& rhs) { return rhs; });
}
